...
| Tianyu Zhao | TA dashboard, profile, CV, permanent PDF link, apply/withdraw flow | `UpdateProfileServlet.java`, `UploadCvServlet.java`, `DownloadCvServlet.java`, `SharedPdfServlet.java`, `PdfShareService.java`, `ApplyServlet.java`, `WithdrawApplicationServlet.java`, `dashboard-ta.jsp` | A TA updates skills, uploads a CV, opens the permanent protected PDF link, applies for a recommended job, and sees the application in history. |
