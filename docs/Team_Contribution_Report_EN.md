...
| Haopeng Jin | Recommendation Engine, Application, PDF Token Sharing, Sprint 4 Security & Availability | 16 files | ~2,250 |
...
| Haopeng Jin | Recommendation engine, application lifecycle, search/filter, permanent PDF token sharing, Sprint 4 security and high-availability hardening | `RecommendationService.java`, `ApplicationService.java`, `JobDetailServlet.java`, `DashboardServlet.java`, `AppServlet.java`, `JsonFileStore.java`, `IdCounterRepository.java`, `LogoutServlet.java`, `PdfShareService.java`, `SharedPdfServlet.java`, `scripts/load_test_recruitassist.py` | Built the explainable matching and critical state-change layer. The recommendation engine scores TA-job pairs across six dimensions and explains matched/missing skills. The application service prevents duplicate active applications, updates statuses, logs actions, closes full jobs, and now supports CSRF protection, POST logout, synchronized registration, cross-process file locks, permanent protected PDF token links, and load-test support. | Demo: sign in as `alice.ta`, compare recommendation percentages and missing skills, apply to a job, then sign in as `mo.chen` and accept/reject the application. Security evidence: inspect hidden `csrfToken`, open a protected `/pdf/share?token=...` link, and show `/health`; load evidence: run `scripts/load_test_recruitassist.py --scenario readonly`. |
